...
| Native SDK Windows directory package and launch | **LOCAL PASS ONLY** | A fresh Windows directory package from `apps/desktop-native/package/windows` is checked, built, packaged, and launch-smoked with local Native CLI **observed** at 0.5.4 and Zig 0.16.0. Evidence records the exact `bin/creature-os-go-agent.exe` SHA-256, a responsive top-level window titled exactly `Creature OS - Go Agent`, and bounded process-tree cleanup. This is not an installed-app or human-interaction check, signing/keyring evidence, or macOS/Linux evidence. Its required `gpu_surface` is the Native SDK canvas renderer, not a WebView; no web, network, filesystem, shell, or credentials permission is declared. |
