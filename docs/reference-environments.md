...
- **Native library compilation:** PARTIAL — [native build evidence workflow](../.github/workflows/native-build.yml) builds, tests, smokes, and retains the host-native Zig dynamic library with per-OS library/checksum/evidence artifacts. This remains PARTIAL until successful workflow runs are recorded.
