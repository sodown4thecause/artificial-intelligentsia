...
| Native library compilation | PARTIAL | The [native build evidence workflow](../../.github/workflows/native-build.yml) builds, tests, smokes, and retains the host-native Zig dynamic library with per-OS library, checksum, and evidence artifacts. This remains PARTIAL until successful workflow runs are recorded. |
