...
import { NativeShortcuts, ShortcutModifiers } from "../../src/native/bridge.js";
...
test("shortcut service registers, invokes, and unregisters local shortcuts", () => {
  ...
  const shortcuts = new NativeShortcuts({ target });
  let invoked = 0;
  assert.equal(shortcuts.register(ShortcutModifiers.Control, "k", "palette", () => { invoked += 1; }), false);
