// PRD §13.1 — Permission classes & default approval matrix.
import type { PermissionClass } from '../types';

export interface PermissionPolicy {
  /** Whether an action of this class may run without explicit user approval. */
  autoApprovable: boolean;
  /** Whether the action may be auto-approved only via an explicit saved rule. */
  ruleGated: boolean;
  /** Whether the action can be permitted at all (destructive may be prohibited). */
  permitted: boolean;
}

export const DEFAULT_POLICY: Record<PermissionClass, PermissionPolicy> = {
 ...
 */
export function classifyAction(action: string, _args: Record<string, unknown> = {}): PermissionClass {
...
export function requiresApproval(action: string, args: Record<string, unknown>, explicitRule = false): boolean {
  const cls = classifyAction(action, args);
  const policy = DEFAULT_POLICY[cls];
  if (!policy.permitted) return true;
  if (cls === 'read-only') return false;
  if (policy.ruleGated) return !explicitRule;
  return true;
}
