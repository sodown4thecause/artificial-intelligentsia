// WRITE-002 — Tone detection (ESTIMATES only, not objective judgments).
export type ToneDimension = 'confidence' | 'formality' | 'warmth' | 'directness' | 'urgency' | 'ambiguity';

const MARKERS: Record<ToneDimension, { pos: RegExp[]; neg: RegExp[] }> = {
...
export type ToneEstimate = Record<ToneDimension, number>; // 0..1

export function detectTone(text: string): { estimate: ToneEstimate; disclaimer: string } {
  const estimate = {} as ToneEstimate;
  for (const dim of Object.keys(MARKERS) as ToneDimension[]) {
    const { pos, neg } = MARKERS[dim];
    let score = 0.5;
    for (const re of pos) if (re.test(text)) score += 0.15;
    for (const re of neg) if (re.test(text)) score -= 0.15;
