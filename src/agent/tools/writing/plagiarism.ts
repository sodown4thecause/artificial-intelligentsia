// WRITE-011 — Plagiarism. Distinguish quotation/citation/common-phrasing/likely-unattributed. Never label as plagiarist from score alone.
export interface PlagMatch {
  passage: string;
  source: string;
  kind: 'quotation' | 'citation' | 'common-phrasing' | 'likely-unattributed';
  similarity: number;
}

export function checkPlagiarism(text: string, sources: { id: string; content: string }[]): PlagMatch[] {
  const matches: PlagMatch[] = [];
  ...
  for (const s of sources) {
    ...
    if (sim > 0.1) {
      ...
      const kind: PlagMatch['kind'] = quoted ? 'quotation' : cited ? 'citation' : sim > 0.5 ? 'likely-unattributed' : 'common-phrasing';
...
export function verdict(matches: PlagMatch[]): string {
  const unattributed = matches.filter((m) => m.kind === 'likely-unattributed');
  if (unattributed.length === 0) return 'No unattributed overlap detected. Similarity alone does not indicate plagiarism.';
