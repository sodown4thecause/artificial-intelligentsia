// WRITE-004 — Sentence/paragraph rewrites preserving protected tokens (names, numbers, citations, links, terms).
export type RewriteMode =
  | 'clarity' | 'concision' | 'fluency' | 'simplification' | 'expansion'
  | 'professional' | 'friendly' | 'persuasive' | 'user-instruction';
...
export function extractProtected(text: string): string[] {
  const found: string[] = [];
...
export function rewrite(text: string, mode: RewriteMode, instruction?: string): { result: string; protectedPreserved: boolean } {
  ...
  switch (mode) {
    ...
    case 'user-instruction':
      ...
      if (instruction) result = `[${mode}: ${instruction}] ${result}`;
      break;
  }
