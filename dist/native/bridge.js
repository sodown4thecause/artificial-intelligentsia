...
function encryptedValue(value, key) {
...
function decryptedValue(value, key) {
    const [nonceText, tagText, ciphertextText] = value.split(".");
    if (!nonceText || !tagText || !ciphertextText)
        return undefined;
    try {
...
export class NativeCache {
    ...
    set(key, value) {
        ...
        if (this.native && this.native.cacheSet(key, value) !== 0)
            throw new Error("Unable to write native cache");
    }
    get(key) {
        if (this.native) {
            ...
            if (this.native.cacheGet(key, pointer, length) === 0) {
                ...
                try {
                    const bytes = koffi.decode(resultPointer, koffi.array("uint8_t", byteLength));
    ...
    delete(key) {
        ...
        if (this.native && this.native.cacheDelete(key) !== 0)
            throw new Error("Unable to delete native cache entry");
    }
